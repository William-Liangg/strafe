...
export function useIntegrationStatus() {
  return useSWR<any>(
    ...
    async () => {
      ...
      const backendStatus = await res.json()

      try {
        const gcalRes = await fetch('/api/integrations/google-status')
        if (gcalRes.ok) {
          const gcalStatus = await gcalRes.json()
          backendStatus.google_calendar = gcalStatus
        }
      } catch {
        // Ignore
      }
...
export function useExpertiseGraph() {
  return useSWR<any>(`${API_BASE}/expertise/graph`, fetcher, {
    refreshInterval: 60000,
  })
}

export function useExpertiseSyncStatus(polling?: boolean) {
  return useSWR<any>(`${API_BASE}/expertise/sync/status`, fetcher, {
